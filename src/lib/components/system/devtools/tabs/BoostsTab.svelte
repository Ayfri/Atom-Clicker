...
<div class="flex flex-col gap-4">
	...
	<div class="rounded-lg bg-black/20 p-4 border border-white/5">
		...
		<p class="mt-2 text-sm text-white/60">
			Manage currency boosts from generator levels. Each point adds <span class="font-bold text-yellow-300">+10%</span> production.
		</p>
		<div class="mt-4 flex justify-end">
			<button
				...
				onclick={() => {
					Object.keys(gameManager.currencyBoosts).forEach(key => {
						gameManager.currencyBoosts[key as CurrencyName] = 0;
					});
				}}
			>
				<RefreshCcw size={14} />
			...
			<div class="flex items-center justify-between gap-3 rounded-lg bg-white/5 p-3 border border-white/5 transition hover:bg-white/10">
				...
				<div class="flex items-center gap-2">
					<button
						...
						disabled={points <= 0}
						onclick={() => {
							gameManager.currencyBoosts[currencyName] = 0;
						}}
					...
					<button
						class="flex h-7 w-7 items-center justify-center rounded bg-white/10 text-white/60 transition hover:bg-white/20 hover:text-white disabled:cursor-not-allowed disabled:opacity-30"
						disabled={gameManager.boostPointsAvailable <= 0 || points >= 20}
