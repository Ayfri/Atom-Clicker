<script lang="ts">
	...
	const groups: { label: string; tabs: SettingsTab[] }[] = [
		{
			label: 'Account',
			tabs: [
				{ component: Profile, description: 'Your account, name and picture', icon: User, id: 'profile', label: 'Profile' },
	...
	/** Lives in `ui` so anything, like the Profile's stats link, can switch tabs through `ui.openSettings`. */
	const shownTab = $derived(!ui.activeTab || (!narrow.current && ui.activeTab === 'home') ? (narrow.current ? 'home' : 'profile') : ui.activeTab);
	...
	{#snippet header()}
		<div class="flex min-w-0 flex-1 items-center gap-1">
			{#if current && narrow.current}
				<button aria-label="Back to settings" class="-ml-2 flex size-10 items-center justify-center rounded-lg text-white/60 transition-colors hover:text-white" onclick={() => (ui.activeTab = 'home')}>
					<ChevronLeft size={24} />
				</button>
			{/if}
